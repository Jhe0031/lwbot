module.exports.run = (client, message, args) => {
  ...
  try {
    ...
    } else if (message.mentions.users.size < 1 && args) {
      try {
        client.fetchUser(message.mentions.users.first().id).then(u => {
          message.channel.send(new Discord.RichEmbed()
            .setTitle(u.tag)
            .setDescription(u.id)
            .setThumbnail(u.avatarURL)
            .setColor(`0x59D851`)
            .setFooter(`You're seeing this because I could not find the user in any shared servers.`)
          );
        });
      } catch (e) {
        message.channel.send(`:x: Could not find the user!`);
      }
