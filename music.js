...
client.on('message', async msg => {
	...
	if (msg.content.startsWith(`${PREFIX}play`)) {
		...
		if (url.match(/^https?:\/\/(www.youtube.com|youtube.com)\/playlist(.*)$/)) {
			...
			try {
				if(args[1].startsWith('http://www.youtube.com/') || args[1].startsWith('https://www.youtube.com/') || args[1].startsWith('https://youtube.com/') || args[1].startsWith('http://youtube.com/')) {var video = await youtube.getVideo(args[1]);} else {var video = await youtube.getVideo(url);}
			} catch (error) {
				try {
